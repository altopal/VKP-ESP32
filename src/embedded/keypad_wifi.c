                          ...
                          int32_t event_id, void* event_data)
{
    ...
    if (event_base == WIFI_PROV_EVENT) {
...
#ifdef CONFIG_EXAMPLE_RESET_PROV_MGR_ON_FAILURE
        wifi_event_sta_disconnected_t* de = (wifi_event_sta_disconnected_t*) event_data;
        if (de->reason == WIFI_REASON_4WAY_HANDSHAKE_TIMEOUT) {
           retries++;
           if (retries >= CONFIG_EXAMPLE_PROV_MGR_MAX_RETRY_CNT) {
        ...
        ESP_LOGI(TAG, "Unknown wifi event %d %d", event_base == WIFI_EVENT, event_id);
...
{
    uint8_t eth_mac[6];
    const char *ssid_prefix = "KEYPAD_";
...
void wifi_connect()
{
    ...
    if (CONFIG_EXAMPLE_RESET_PROVISIONED || !provisioned) {
